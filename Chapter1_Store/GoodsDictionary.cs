...
namespace NapilnikStudyCourse.Chapter1_Store
{
	public class GoodsDictionary
	{
		...
		public void Add(Good good, int count)
		{
			...
			if (count <= 0)
				throw new ArgumentOutOfRangeException("ошибка в количестве добавляемых товаров");
			
			if (_goods.ContainsKey(good)) // TODO check working ContainsKey
				_goods[good] += count;
			else
		...
		{
			if (_goods.Count == 0)
				Console.WriteLine("в списке нет товаров");
			else
				foreach(var e in _goods) Console.WriteLine(e.Key.Name + " " + e.Value);
		}
